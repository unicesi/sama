...
public class DTSamaController extends Controller{
	...
	public DTSamaController(){
		...
		registerEventTypes(DTEvent.CONSULTAR_MATERIA);


		registerEventTypes(DTEvent.CONSULTAR_BLOQUE);
		registerEventTypes(DTEvent.CONSULTAR_BLOQUE2);
		...
		registerEventTypes(DTEvent.ACTUALIZAR_REPORTE);
										...
										if(event.getType().equals(DTEvent.SELECCIONAR_COMPETENCIA_PROFESIONAL)){
											...
											if(event.getType().equals(DTEvent.SELECCIONAR_COMPETENCIA_TRANSVERSAL)){
												...
												if(event.getType().equals(DTEvent.ASOCIAR_MATERIA)){
													...
													if(event.getType().equals(DTEvent.CONSULTAR_MATERIA)){
														...
														if(event.getType().equals(DTEvent.CONSULTAR_BLOQUE)){
															...
															if(event.getType().equals(DTEvent.CONSULTAR_BLOQUE2)){
																BloqueModel pM=Registry.get("bloqueSeleccionadoFiltro2");
																								...
																								if(event.getType().equals(DTEvent.CAMBIAR_PANELES)){
																									...
																									if(opcion.equals("Filtro-Bloques")){
																										...
																										if(panelFiltro.getGridBloquesSeleccionados().getStore().getCount()>0 && (panelFiltro.getGridProfesionalesSeleccionadas().getStore().getCount()>0 ||panelFiltro.getGridTransversalesSeleccionadas().getStore().getCount()>0)){
																											...
																											panelAsociar.actualizarBloques(listaFiltrados);
																														...
																														if(event.getType().equals(DTEvent.REFRESCAR_LINEA_TRANSVERSAL)){
																															...
																															if(event.getType().equals(DTEvent.REFRESCAR_MATERIAS_FILTRO)){
																																...
																																if(lineas!=null){
																																	...
																																	PanelAsociarCompetenciasEspecificas filtroMateria = Registry.get("panelAsociar");
																																	filtroMateria.actualizarMaterias(listaAsociados);
																																...
																																if(event.getType().equals(DTEvent.REFRESCAR_MATERIAS_BLOQUES)){
																																	...
																																	if(event.getType().equals(DTEvent.ACTUALIZAR_REPORTE)){
																																		
																																		PanelAsociarCompetenciasEspecificas panel1 = (PanelAsociarCompetenciasEspecificas) Registry.get("panelAsociar");
																																		panel1.crearReporte(2, (String) Registry.get("idPrograma"));
																																		
																																	}else{
																																		
																																		if(event.getType().equals(DTEvent.ACTUALIZAR_REPORTE_BLOQUES)){
																																			
																																			PanelAsociarCompetenciasEspecificasBloque panel1 = (PanelAsociarCompetenciasEspecificasBloque) event.getData();
																																			panel1.crearReporte(1, (String) Registry.get("idPrograma"));
																																			System.out.println("Entra al evento. IdPrograma: " + (String)Registry.get("idPrograma")+", Panel: " + panel1.getId());
