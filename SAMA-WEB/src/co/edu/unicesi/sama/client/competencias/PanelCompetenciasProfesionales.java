...
/**
 * Esta clase esta encargada de crear y mostrar el panel de competencias profesionales.
 */
public class PanelCompetenciasProfesionales extends LayoutContainer {
	...
	/**
	 * Constructor encargado de agregar los componentes e inicializarlos
	 */
	public PanelCompetenciasProfesionales() {
		setSize("770", "462");
		...
		Text txtMateriasDisponibles = new Text("Lineas profesionales disponibles");
		...
		btnAgregarLinea.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				...
				Dispatcher.forwardEvent(DTEvent.ASOCIAR_LINEA_PROFESIONAL);
			}
		});
		...
		btnQuitarLinea.addListener(Events.Select, new Listener<ButtonEvent>() {
			public void handleEvent(ButtonEvent e) {
				Dispatcher.forwardEvent(DTEvent.DESASOCIAR_LINEA_PROFESIONAL);
		...
		eventoSeleccionarCompetenciaProfesional();
	...
	/**
	 * Este metodo se encarga de actualizar las listas de competencias profesionales.
	 */
	private void actualizarLista(){
		
		
		busquedaService.buscarCompetenciaPorPrograma("1", String.valueOf(programa.getIdPrograma()),new AsyncCallback<ArrayList<CompetenciaTerminalBO>> (){
			@Override
			...
			public void onSuccess(ArrayList<CompetenciaTerminalBO> result) {
				...
				Dispatcher.forwardEvent(DTEvent.REFRESCAR_COMPETENCIAS_PROFESIONALES, result);
			}
		});
...
//			gridAsociadas.getStore().getLoader().load();
	 ...
	 void eventoSeleccionarCompetenciaProfesional(){
		...
		grid.addListener(Events.CellClick, new Listener<GridEvent<CompetenciaTerminalModel>>(){
			...
			public void handleEvent(GridEvent<CompetenciaTerminalModel> be) {
				...
				Dispatcher.forwardEvent(DTEvent.SELECCIONAR_COMPETENCIA_PROFESIONAL, pM);
	...
	public void cargarLineasProf(){
		...
		busquedaService.buscarLineaDeCompetenciaPorCompetencia(Registry.get("idPrograma")+"", Registry.get("idCompetenciaProfesional")+"", new AsyncCallback<ArrayList<CompetenciaEspecificaBO>>() {
			...
			public void onSuccess(ArrayList<CompetenciaEspecificaBO> result) {
				...
				if(result != null){
					...
					Dispatcher.forwardEvent(DTEvent.REFRESCAR_LINEA_PROFESIONAL, result);
